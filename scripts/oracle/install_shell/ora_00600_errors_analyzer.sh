#!/bin/sh
#
# $Header: opsm/cvutl/pluggable/unix/ora_00600_errors_analyzer.sh /st_has_12.1/1 2014/04/24 22:55:53 nvira Exp $
#
...
then
  if [ $count -le $expectedCount ]
    then
      resultSet=${resultSet}"<RESULT><STATUS>SUCC</STATUS><ROW_ID>$instance_id</ROW_ID><COLLECTED>ORA-00600_COUNT=$count</COLLECTED><EXPECTED>ORA-00600_COUNT=$expectedCount</EXPECTED><TRACE>No ORA-00600 errors found in alert log on node $nodeName</TRACE><NLS_MSG><FACILITY>Prve</FACILITY><ID>2882</ID><MSG_DATA><DATA>$instance_id</DATA></MSG_DATA></NLS_MSG></RESULT>"
  else
      resultSet=${resultSet}"<RESULT><STATUS>VFAIL</STATUS><ROW_ID>$instance_id</ROW_ID><COLLECTED>ORA-00600_COUNT=$count</COLLECTED><EXPECTED>ORA-00600_COUNT=$expectedCount</EXPECTED><TRACE>ORA-00600 errors found in alert log on node $nodeName</TRACE><NLS_MSG><FACILITY>Prve</FACILITY><ID>2883</ID><MSG_DATA><DATA>$instance_id</DATA></MSG_DATA></NLS_MSG></RESULT>"
  fi   
else
  resultSet=${resultSet}"<RESULT><STATUS>EFAIL</STATUS><ROW_ID>$instance_id</ROW_ID><EXEC_ERROR>Error while checking for ORA-00600 errors in alert log</EXEC_ERROR><TRACE>Error while checking for ORA-00600 errors in alert log on node $nodeName</TRACE><NLS_MSG><FACILITY>Prve</FACILITY><ID>2884</ID><MSG_DATA><DATA>$instance_id</DATA></MSG_DATA></NLS_MSG></RESULT>"
fi   
